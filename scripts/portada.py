...
TITULO = "OMNI Guard"
LEMA = "De una cámara que solo graba a una que avisa"
FRASE = ("Una cámara graba; nadie la mira. Lo que cambia el resultado es avisar mientras pasa, y cobrar sin cuaderno.")
...
def main() -> int:
    ...
    PASOS = [
        ("01", "video", "Entra", "La cámara que ya está",
         f"0 €", f"en instalación nueva"),
        ("02", "cpu", "Detecta", "YOLO11n en la GPU",
         f"{fps}", f"mAP@50 de {acierto}"),
        ("03", "escudo", "Vigila", "Intrusión y merodeo",
         f"3", f"reglas configurables"),
        ("04", "reloj", "Cobra", "Por tiempo en la plaza",
         f"S/", f"sin apuntar nada a mano"),
    ]
    ...
    n = len(med)
    for i, (et, val) in enumerate((("Pruebas", f"20 comprobaciones"), ("Modelos medidos", f"{n}"), ("Rostros publicados", f"ninguno"), ("Datos simulados", f"ninguno"))):
        x = m + i * ((W - 2 * m) / 4)
